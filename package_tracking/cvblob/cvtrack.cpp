		...
		if (mode)
		{
			for (CvTracks::const_iterator it = tracks.begin(); it != tracks.end(); ++it)
			{
				...
				if (!it->second->inactive)
				{
					...
					buffer << it->first;
					cv::Mat imgg(imgDest);
					//cv::putText(imgg, buffer.str().c_str(), cv::Point((int)it->second->centroid.x + 10, (int)it->second->centroid.y),cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255), 2);
				...
				if (it->second->inactive)
					//When inactive: give the black box
					cvRectangle(imgDest, cvPoint(it->second->minx, it->second->miny), cvPoint(it->second->maxx - 1, it->second->maxy - 1), CV_RGB(0., 0., 255.), 1);
				else
					//When active, provide the blue box
					cvRectangle(imgDest, cvPoint(it->second->minx, it->second->miny), cvPoint(it->second->maxx - 1, it->second->maxy - 1), CV_RGB(0., 0., 255.), 2);
