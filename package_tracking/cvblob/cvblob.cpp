...
namespace cvb
{
	...
	void cvFilterByArea(CvBlobs &blobs, unsigned int minArea, unsigned int maxArea)
	{
		...
		{
			CvBlob *blob = (*it).second;
			// 筛选 if ((blob->area<minArea) || (blob->area>maxArea))
			// 高宽比小于1.2的blob被舍掉
			if ((blob->area < minArea) || (blob->area > maxArea) || ((blob->maxy - blob->miny) / (blob->maxx - blob->minx) < 1.2))
			{
				cvReleaseBlob(blob);
	...
	void cvRenderBlob(const IplImage *imgLabel, CvBlob *blob, IplImage *imgSource, IplImage *imgDest, unsigned short mode, CvScalar const &color, double alpha)
	{
		...
		if (mode)
		{
			...
			if (mode&CV_BLOB_RENDER_BOUNDING_BOX)
				cvRectangle(imgDest, cvPoint(blob->minx, blob->miny), cvPoint(blob->maxx - 1, blob->maxy - 1), CV_RGB(255., 0., 0.), 2);
			...
			if (mode&CV_BLOB_RENDER_ANGLE)
			{
				...
				// Green light throught the centroid is the angle line
				//cvLine(imgDest,cvPoint(int(x1),int(y1)),cvPoint(int(x2),int(y2)),CV_RGB(0.,255.,0.),2);
			}

			if (mode&CV_BLOB_RENDER_CENTROID)
			{
				cvLine(imgDest, cvPoint(int(blob->centroid.x) - 3, int(blob->centroid.y)), cvPoint(int(blob->centroid.x) + 3, int(blob->centroid.y)), CV_RGB(0., 0., 255.), 2);
				cvLine(imgDest, cvPoint(int(blob->centroid.x), int(blob->centroid.y) - 3), cvPoint(int(blob->centroid.x), int(blob->centroid.y) + 3), CV_RGB(0., 0., 255.), 2);
			}
		}
